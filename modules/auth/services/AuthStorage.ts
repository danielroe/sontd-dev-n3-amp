...
import { AuthConfig, User, AuthTokens } from '../types';
...
export default class AuthStorage {
  ...
  constructor({ authConfig }: { authConfig: AuthConfig }) {
    ...
    this.accessTokenCookie = useCookie(
      AuthStorage.ACCESS_TOKEN_STORAGE_KEY,
      this.cookieOptions
    );
    this.refreshTokenCookie = useCookie(
      AuthStorage.REFRESH_TOKEN_STORAGE_KEY,
      this.cookieOptions
    );
    this.persistent = useCookie(AuthStorage.FLAG_PERSISTENT_KEY, {
      expires: undefined,
    });
  ...
  setPersistent(value: boolean) {
    ...
    this.persistent = useCookie(AuthStorage.FLAG_PERSISTENT_KEY, cookieOptions);
